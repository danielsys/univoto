...
class AdminUser {
    ...
    public function ExeUpdate($UserId, array $Data) {
        ...
        if (!$this->Data['users_password']):
            unset($this->Data['users_password']);
        endif;
    ...
    public function ExeDelete($UserId) {
        ...
        $readUser = new Read;
        $readUser->ExeRead(self::Entity, "WHERE idusers = :id", "id={$this->User}");

        if (!$readUser->getResult()):
            $this->Error = ['Oppsss, você tentou remover um usuário que não existe no sistema!', WS_ERROR];
            $this->Result = false;
        elseif ($this->User == $_SESSION['userlogin']['idusers']):
