...
class Session {
    ...
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('online_siteviews_agent', "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResult()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('online_siteviews_agent', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResult()[0]['agent_views'] + 1];
            $updateAgent = new Update;
            $updateAgent->ExeUpdate('online_siteviews_agent', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
    ...
    private function setUsuario() {
        ...
        $userCreate->ExeCreate('online', $sesOnline);
    }
    ...
    private function UsuarioUpdate() {
        ...
        $userUpdate->ExeUpdate('online', $ArrOnlone, "WHERE online_session = :ses", "ses={$_SESSION['useronline']['online_session']}");

        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('online', 'WHERE online_session = :onses', "onses={$_SESSION['useronline']['online_session']}");
